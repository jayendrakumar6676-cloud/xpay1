...
export const Route = createFileRoute("/coding/$examId")({
  head: () => ({
    meta: [
      { title: "Coding Round | XPay Portal" },
      { name: "description", content: "Multi-language proctored coding round." },
    ],
  }),
  component: CodingExam,
});
...
function CodingExam() {
  ...
  const [state, setState] = useState<Record<string, CodeState>>(() => {
    ...
    for (const qq of CODING_QUESTIONS) {
      init[qq.id] = { language: "python", code: SUPPORTED_LANGUAGES[0].starter };
    }
    return init;
  });
  ...
  const submit = useCallback(async () => {
    ...
    for (const qq of questions) {
      ...
      totalMarks += earned;
      totalPossible += qq.marks;
      results.push({
        questionId: qq.id,
        language: st.language,
        code: st.code,
        passed,
        total: qq.testCases.length,
        marksEarned: earned,
        perCase,
      });
    }
    saveCodingSubmission(candidateEmail, {
      examId,
    ...
    setSubmitting(false);
    setPhase("submitted");
  }, [questions, state, candidateEmail, examId, violations]);

  const flagViolation = useCallback((reason: string) => {
    setViolations((v) => {
      const next = v + 1;
      if (next >= MAX_VIOLATIONS) {
        toast.error("Coding round auto-submitted: too many violations.");
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      // Allow Ctrl+C / Ctrl+V / Ctrl+A / Ctrl+Z / Ctrl+Y / Ctrl+S inside the editor textarea only
  ...
  const runSampleTests = async () => {
    ...
    setState((s) => ({
      ...s,
      [q.id]: { ...s[q.id], lastRun: { passed, total: visible.length, logs: logs.join("\n") } },
    }));
    setRunning(false);
    ...
    <div ref={containerRef} className="min-h-screen select-none bg-background">
      ...
      {phase === "blocked" && (
        <CenterCard>
          ...
          <p className="mt-3 text-sm text-muted-foreground">
            You have already submitted the Coding Round. Each test can be attempted only once.
          </p>
          <Link to="/dashboard">
          ...
          <Card className="max-w-2xl w-full glass shadow-brand">
            <CardContent className="p-8">
              ...
              <ul className="mt-6 space-y-2 text-left text-sm text-muted-foreground">
                <li>• Each question is graded against <strong className="text-foreground">visible + hidden test cases</strong>. Partial credit is given.</li>
                <li>• Pick any language per question. Read from <strong>STDIN</strong>, write to <strong>STDOUT</strong>.</li>
                <li>• Use <strong>Run Sample Tests</strong> to check your solution before submitting.</li>
                ...
                <li>• Only <strong>one</strong> attempt. Scores are not shown — invigilator reviews submissions.</li>
              </ul>
      ...
      {phase === "permissions" && (
        <CenterCard>
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Required for proctoring throughout the coding round.
          </p>
        ...
        <div className="mx-auto max-w-[1400px] px-4 py-4">
          <header className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-2xl glass px-4 py-3 shadow-sm">
            <div className="flex items-center gap-3">
              <Logo className="h-8" />
              <Badge variant="secondary">Coding Round</Badge>
            </div>
              ...
              <CardContent className="p-6 max-h-[75vh] overflow-y-auto">
                ...
                {q.sample.map((s, i) => (
                  <div key={i} className="mt-3 rounded-lg border border-border bg-muted/30 p-3 text-xs font-mono">
                    ...
                    {s.explanation && (<>
                      <div className="mt-2 text-muted-foreground">Explanation</div>
                      <pre className="whitespace-pre-wrap font-sans">{s.explanation}</pre>
                    </>)}
                  </div>
                ))}
              </CardContent>
      ...
      {phase === "submitted" && (
        <CenterCard>
          ...
          <p className="mt-3 text-sm text-muted-foreground">
            Your code has been recorded and graded against all test cases. Results will be shared by the invigilator.
          </p>
          <Link to="/dashboard">
            <Button className="mt-6 h-11 w-full bg-brand-gradient border-0 text-white font-semibold">Back to Dashboard</Button>
          </Link>
        </CenterCard>
      )}
    </div>
